...
class RecordViewController: UIViewController {
    ...
    var watermarkLoaded: Bool = false {
        didSet {
            if !watermarkLoaded {
                self.updateWatermarksButton.isEnabled = false
                self.recordVideoButton.isEnabled = false
                self.shotPhotoButton.isEnabled = false
                self.galleryButton.isEnabled = false
                self.updateWatermarksButton.isEnabled = false
            } else if cameraReady {
                self.updateWatermarksButton.isEnabled = true
                self.recordVideoButton.isEnabled = true
                self.shotPhotoButton.isEnabled = true
    ...
    func exportImageWithWatermark(_ image: UIImage) {
        ...
        let watermark = watermarkImages[watermarkCount]!
            ...
            NetworkLayer.getWatermarkURLs(user: userId, key: userSecure) { finished, watermarks in
                ...
                for watermark in watermarks {
                    blockOperation.addExecutionBlock {
                        guard let url = URL(string: watermark) else { return }
                        if let data = try? Data(contentsOf: url) {
                            let image = UIImage(data: data)

                            self.watermarkImages.append(image)
                        }
                    }
                }
                blockOperation.completionBlock = {
                    if self.watermarkImages.count > 1 {
                        DispatchQueue.main.async {
    ...
    private func updateWatermark() {
        if let image = watermarkImages[watermarkCount] {
            DispatchQueue.main.async {
    ...
    func changeWatermark(_ sender: UIButton) {
        ...
        case prevWatermarkButton:
            ...
            if watermarkCount == 0 {
                prevWatermarkButton.isEnabled = false
            } else {
                nextWatermarkButton.isEnabled = true
            }
            break
        case nextWatermarkButton:
            if watermarkCount < watermarkImages.count - 1 {
                watermarkCount += 1
            }
            if watermarkCount == watermarkImages.count - 1 {
                nextWatermarkButton.isEnabled = false
            } else {
                prevWatermarkButton.isEnabled = true
            }
            break
        default: break
        }
